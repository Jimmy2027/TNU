...
map_t   %RESULTS TABLE FOR MAP


%COMPARISON OF THE INFLUENCE OF DIFFERENT VALUES OF THE VARIANCE OF THE
%PRIOR:
%FROM THE MAP TABLE IT'S POSSIBLE TO SEE THAT THE SMALLER THE VARIANCE OF
%THE PRIOR, THE SMALLER THE TOTAL SUM OF THE PARAMETERS. THAT IS BECAUSE A
%GAUSSIAN PRIOR ON THE PARAMETERS MEANS REGULARIZING A REGRESSION PROBLEM
%(RIDGE REGRESSION), AND THE REGULARIZING FACTOR LAMBDA, IS THE INVERSE OF
%THE VARIANCE OF THE PRIOR. HENCE THE SMALLER THE PRIOR VARIANCE THE BIGGER
%IS OTHE REGULARIZING STRENGTH, AND THE SMALLER WILL BE THE TOTAL SUM OF
%THE ESTIMATED PARAMETERS. I HIGHLIGHT THE FACT THAT THE SUM OF THE
%PARAMETERS IN SMALLER, AND NOT THE SINGLE VALUES, IN FACT FROM THE TABLE
%IS POSSIBLE TO SEE THAT IN THE CASE OF VAR=10 THERE ARE SOME PARAMETERS
%THAT ARE SMALLER RESPECT TO THE VERSION WITH VAR=0.1 OR 0.001.
%AS EXPECTED THE LOG LIKELIHOOD VALUE OF THE MLE SOLUTION IS ALWAYS MUCH
%HIGHER THAN THE LOG LIKELIHOOD VALUE OF THE MAP SOLUTION; THIS IS DUE THE
%FACT THAT IN MLE WE HAVE NO REGULARIZATION, AND THE GOAL IS ONLY TO
%MAXIMIZE THE LOG LIKELIHOOD FUNCTION, AND THE VALUE OF THE PARAMETERS IS
%FITTED ARBITRARLY IN ORDER TO GET THE MAXIMUM VALUE OF THE LOG-L. IN
%CONTRAST, IN THE MAP CASE THE GOAL IS TO MAXIMIZE THE POSTERIOR, BUT THE
%PARAMETER ARE CONSTRAINED, HENCE THE PARAMETERS WILL NOT BE ESTIMATED WITH
%THE ONLY GOAL TO MAXIMIZE THE FUNCTION.
...
function L = likelihoo(yn, A) %yn is vector of noisy observations y_hat, sigma noise std., a is vector of connectivity parameters
    ...
    P.B(2,1) = A(4);
    P.C(1,1) = A(5);
    L=0; %initial value of log likelihood
...
function JP = joint(yn, A, var_p) %A is a vector whose entries correspond to a11,a21,a22,b21,c11 in this order, var_p is the variance of the parameters
