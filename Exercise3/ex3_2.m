...
for i=1:length(b)
    ...
    plot(axis,xo(2,2:end), 'b-', 'LineWidth' ,2);
    legend(['$x_{2}$'], 'Interpreter', 'latex');
    title("Second state with b=" + string(b(i)));

    nexttile;
  
    plot(axis,yo.two, 'g-', 'LineWidth' ,2);
    legend(['$y_{2}$'], 'Interpreter', 'latex');
    title("Second signal for b=" + string(b(i)));

end

title(t, "PLOTS FOR B_3");
